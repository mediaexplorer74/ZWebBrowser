...
namespace ICSharpCode.SharpZipLib.Zip
{
  public class ZipEntry : ICloneable
  {
    ...
    {
      get => (this.known & ZipEntry.Known.CompressedSize) == ZipEntry.Known.None ? -1L : (long) this.compressedSize;
      set
      {
    ...
    public long Crc
    {
      get => (this.known & ZipEntry.Known.Crc) == ZipEntry.Known.None ? -1L : (long) this.crc & (long) uint.MaxValue;
      set
      {
    ...
    internal CompressionMethod CompressionMethodForHeader => this.AESKeySize <= 0 ? this.method : CompressionMethod.WinZipAES;
    ...
    private void ProcessAESExtraData(ZipExtraData extraData) => throw new ZipException("AES unsupported");
