...
namespace ICSharpCode.SharpZipLib.Zip
{
  public class ZipFile : IEnumerable, IDisposable
  {
    ...
    private void ReadEntries()
    {
      ...
      if (length2 > 0U)
      {
        byte[] numArray = new byte[length2];//byte[(IntPtr) length2];
    ...
    private long LocateEntry(ZipEntry entry) => this.TestLocalHeader(entry, ZipFile.HeaderTest.Extract);
    ...
    private class ZipUpdate
    {
      ...
      public ZipUpdate(ZipEntry original, ZipEntry updated) => throw new ZipException("Modify not currently supported");
    ...
    private class UncompressedStream : Stream
    {
      ...
      public override void Write(byte[] buffer, int offset, int count) => this.baseStream_.Write(buffer, offset, count);
    }
